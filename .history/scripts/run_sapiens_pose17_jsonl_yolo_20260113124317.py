...
def extract_subject_from_path(video_path: Path) -> str:
    """
    Estrae il nome del soggetto dal path del video.
    
    Assume struttura: .../soggettoNNN/video.mp4
    Restituisce il nome della cartella padre.
    """
...
def main():
    ...
    # Estrai soggetto dal path (es. soggetto002 da .../soggetto002/video.mp4)
    subject_name = extract_subject_from_path(video_path)
    
    # Determine output paths: output_dir/soggettoNNN/nome_video.*
    ...
    output_jsonl = subject_output_dir / f"{output_name}.jsonl"
    output_video = subject_output_dir / f"{output_name}_overlay.mp4"
    ...
    if not args.quiet:
        ...
        print(f"[CONFIG] Subject: {subject_name}")
