...
def process_single_video(
    ...
    output_jsonl = subject_output_dir / "keypoints_jsonl" / f"{video_stem}.jsonl"
    output_video = subject_output_dir / "videos" / f"{video_stem}.mp4"
...
def main():
    ...
    input_group.add_argument(
        "--video", "-v",
        type=Path,
        help="Percorso file video singolo (richiede --subject)",
    )
    input_group.add_argument(
        "--dataset",
        type=Path,
        help="Radice del dataset con struttura: dataset/soggettoNNN/*.mp4",
    )
    
    # Nome del soggetto (richiesto per video singolo)
    parser.add_argument(
        "--subject",
        type=str,
        help="Nome della cartella soggetto (es., soggetto001). Richiesto con --video",
    )
    ...
    parser.add_argument(
        "--output-dir", "-o",
        type=Path,
        help="Directory radice di output (default: data/output)",
    )
    ...
    parser.add_argument(
        ...
        help="File di configurazione per il mapping dei keypoints",
    )
    ...
    parser.add_argument(
        "--stride", "-s",
        type=int,
        help="Processa ogni N-esimo frame",
    )
    parser.add_argument(
        "--device", "-d",
        type=str,
    ...
    parser.add_argument(
        "--yolo-conf",
        type=float,
        help="Soglia di confidenza YOLO",
    )
    parser.add_argument(
        "--box-padding",
        type=float,
        help="Rapporto padding bounding box",
    )
    parser.add_argument(
        "--keypoint-threshold",
        type=float,
        help="Soglia di confidenza keypoint per la visualizzazione",
    )
    ...
    parser.add_argument(
        "--no-video",
        action="store_true",
        help="Salta output video con overlay",
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
        help="Sopprimi output di progresso",
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
        help="Salta video che hanno già output JSONL",
    )
    
    args = parser.parse_args()
    
    # validazione arguments
    ...
    # Parametri di elaborazione (CLI sovrascrive config)
    ...
    # Costruisci lista di video da processare
    ...
    if args.skip_existing:
        ...
        for subject_name, video_path in videos_to_process:
            ...
            if output_jsonl.exists():
                if not args.quiet:
                    print(f"[SKIP] già presente: {output_jsonl}")
            else:
                filtered.append((subject_name, video_path))
        videos_to_process = filtered
    
    if not videos_to_process:
        print("[INFO] Nessun video da processare (tutti saltati o nessuno trovato)")
        sys.exit(0)
    
    # Stampa configurazione per check
    if not args.quiet:
        print("=" * 60)
    ...
    # Inizializza modelli (una sola volta)
    if not args.quiet:
        print(f"[INFO] Caricamento YOLO: {yolo_weights}")
    ...
    if not args.quiet:
        print(f"[INFO] Caricamento Sapiens: {sapiens_checkpoint}")
