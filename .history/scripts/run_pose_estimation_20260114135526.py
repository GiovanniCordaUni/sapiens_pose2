...
Usage:
    ...
    python run_sapiens_pose17_jsonl_yolo.py --video <path> --subject soggetto001

    # Intero dataset
    python run_sapiens_pose17_jsonl_yolo.py --dataset data/input/videos

    # Con config personalizzato
    python run_sapiens_pose17_jsonl_yolo.py --dataset <path> --config <config.yaml>
"""
...
def discover_dataset_videos(dataset_root: Path) -> Generator[tuple[str, Path], None, None]:
    """
    Discover videos in dataset structure: dataset_root/soggettoNNN/*.mp4
...
def process_single_video(
    ...
    """
    Process a single video and save outputs in subject folder.
    
    Output structure:
        output_dir/
            soggettoNNN/
                nome_video.jsonl
                nome_video_overlay.mp4
    """
    ...
    output_video = subject_output_dir / f"{video_stem}_overlay.mp4"
