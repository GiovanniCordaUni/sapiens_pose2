...
class VideoReader:
    ...
    def iter_frames(
        ...
        Args:
            stride: Processa ogni N-esimo frame (default: 1 = tutti i frame)
            start_frame: Inizia da questo indice di frame
            end_frame: Termina a questo indice di frame (esclusivo)
            
        Yields:
            Tuple di (frame_index, frame_bgr)
        """
        if end_frame is None:
            end_frame = self.frame_count
            
        # Punta al frame iniziale se necessario
        if start_frame > 0:
    ...
    def iter_all_with_skip(
        ...
        """
        Itera su tutti i frame, restituendo None per i frame saltati.
        
        Utile quando è necessario tracciare gli indici dei frame ma
        processare solo alcuni frame per risparmiare tempo o risorse.
        
        Args:
            stride: Processa ogni N-esimo frame
            
        Yields:
            Tuple di (frame_index, frame_bgr o None per i frame saltati)
        """
        frame_idx = 0
        while True:
            ret, frame = self._cap.read()
