...
Orchestrates video reading, detection, pose estimation, and output writing.
"""
...
def run_pose_pipeline(
    ...
    """
    Run full pose estimation pipeline on a video.
    
    Args:
        video_path: Input video file path
        output_jsonl: Output JSONL file path for keypoints
        output_video: Output video file path with overlay
        yolo_detector: Configured YOLO person detector
        pose_estimator: Configured Sapiens pose estimator
        keypoint_indices: Indices to select from model output (e.g., 308 -> 17)
        frame_stride: Process every N-th frame
        box_padding: Padding ratio for bounding boxes
