...
class SapiensPoseEstimator:
    ...
    Args:
        checkpoint_path: Path to Sapiens checkpoint
        device: Device to run on ("auto", "cuda", "cpu")
        input_width: Model input width (default: 768)
        input_height: Model input height (default: 1024)
        
    Example:
        estimator = SapiensPoseEstimator("sapiens_0.3b.pt2")
        keypoints, scores = estimator.predict_crop(crop_bgr)
    """
    ...
    def predict_frame(
        ...
        """
        Predict keypoints and transform to frame coordinates.
        
        Args:
            crop_bgr: BGR image crop as numpy array
            crop_box_xyxy: Bounding box [x1, y1, x2, y2] used for cropping
            keypoint_indices: Optional indices to select subset of keypoints
            
        Returns:
            keypoints: Array of shape (K, 2) in frame coordinates
            scores: Array of shape (K,) with confidence scores
        """
