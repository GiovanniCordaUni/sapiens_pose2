...
Funzioni per il caricamento e l'inferenza del modello Sapiens.
"""
...
def get_device(device: str = "auto") -> torch.device:
    """
    Ottiene il dispositivo torch appropriato.
    
    Args:
        device: Specifica del dispositivo - "auto", "cuda", "cpu", o "mps"
...
def load_sapiens(
    ...
    """
    Carica un modello Sapiens TorchScript.
    
    Args:
        checkpoint_path: Percorso al file di checkpoint .pt2
        device: Dispositivo su cui caricare il modello ("auto", "cuda", "cpu", o torch.device)
        
    Returns:
        Tuple di (modello, dispositivo) dove il modello è caricato e in modalità eval
...
class SapiensPoseEstimator:
    ...
    Incapsula il caricamento del modello, il preprocessing, l'inferenza e la decodifica.
    Il checkpoint COCO 17 produce direttamente 17 keypoints in formato COCO,
    nessun mapping aggiuntivo richiesto.
    
    Args:
        checkpoint_path: Percorso al checkpoint Sapiens (COCO 17 nativo)
        device: Dispositivo su cui eseguire ("auto", "cuda", "cpu")
        input_width: Larghezza di input del modello (default: 768)
        input_height: Altezza di input del modello (default: 1024)
    ...
    def num_keypoints(self) -> int | None:
        """Numero di keypoints nell'output del modello (determinato dopo la prima inferenza)."""
        return getattr(self, "_num_keypoints", None)
        
    def predict_heatmaps(self, crop_bgr: np.ndarray) -> np.ndarray:
        """
        Esegue l'inferenza e restituisce le heatmap grezze.
        
        Args:
            crop_bgr: Crop dell'immagine in formato BGR come array numpy
            
        Returns:
            Heatmaps di forma (K, Hm, Wm)
        """
        # Preprocess
    ...
    def predict_crop(
        ...
        """
        Predice i keypoints da un crop nello spazio di input del modello.
        
        Args:
            crop_bgr: Crop dell'immagine in formato BGR come array numpy
            
        Returns:
            keypoints: Array di forma (K, 2) nelle coordinate di input del modello
            scores: Array di forma (K,) con i punteggi di confidenza
        """
    ...
    def predict_frame(
        ...
        # Trasforma nelle coordinate del frame
