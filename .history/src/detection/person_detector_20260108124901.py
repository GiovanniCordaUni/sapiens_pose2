...
Pure, stateless functions for bounding box operations and person selection.
"""
...
def iou_xyxy(box_a: list | np.ndarray, box_b: list | np.ndarray) -> float:
    """
    Compute Intersection over Union (IoU) for two boxes in xyxy format.
...
def clamp_box_xyxy(
    ...
    """
    Clamp a bounding box to stay within frame boundaries.
    
    Ensures the box coordinates are valid (x1 < x2, y1 < y2) and
