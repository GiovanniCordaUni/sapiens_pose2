...
def draw_coco17(
    ...
    # Disegna gli edges dello scheletro
    for a, b in edges:
...
def draw_bbox(
    ...
    """
    Disegna un bounding box su un'immagine.
    
    Args:
        frame_bgr: Immagine BGR di input
        box_xyxy: Coordinate del box come [x1, y1, x2, y2]
        color: Colore BGR per il box
        thickness: Spessore della linea
        
    Returns:
        Immagine con box disegnato (copia dell'input)
    """
    out = frame_bgr.copy()
...
def draw_pose_overlay(
    ...
    """
    Disegna l'overlay completo della posa inclusi scheletro e bounding box opzionale.
    
    Args:
        frame_bgr: Immagine BGR di input
        keypoints_xy: Coordinate dei keypoint di shape (17, 2)
        scores: Punteggi di confidenza dei keypoint di shape (17,)
        box_xyxy: Bounding box opzionale come [x1, y1, x2, y2]
        keypoint_threshold: Confidenza minima per disegnare un keypoint
        keypoint_radius: Raggio dei cerchi dei keypoint
        skeleton_thickness: Spessore delle linee dello scheletro
        keypoint_color: Colore BGR per i keypoint
        ...
        edges: Edges personalizzati dello scheletro
        
    Returns:
        Immagine con overlay completo della posa (copia dell'input)
    """
    out = frame_bgr.copy()
    
    # Disegna il bounding box se fornito
